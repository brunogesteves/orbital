...
<div class="flex items-start w-full gap-x-2 h-auto px-6 mb-5">
    <div class="w-1/2 flex flex-wrap justify-around gap-y-2 h-[552px]">
    ...
    <div class="w-1/2 relative overflow-hidden flex flex-col h-full h-[552px]">
        <div class="sliderArea w-full h-full relative cursor-pointer mb-6 h-full">
            <div id="previewImg" class="hidden bg-slate-552 hover:bg-slate-300 z-10 absolute top-1/2 left-0 rounded-l-md cursor-pointer p-3 rotate-180">
                <img src="/images/icons/arrow-right.png" width="20" />
            </div>
            <div id="nextImg" class="hidden max-sm:invisible bg-slate-552 hover:bg-slate-300 z-10 absolute top-1/2 right-0 rounded-l-md cursor-pointer p-3 rotate-0">
            ...
            <div class="sliderFront text-center text-white w-full">
                <?php foreach ($posts2 as $post) : ?>
                    <a href=<?= $post["slug"] ?>>
                        <div class="relative">
                            <img src=<?= $post["source"] == "Orbital Channel" ? "/images/$post[image]" :  $post["image"] ?> alt="" class="object-cover object-top h-[552px] w-full">
...
<div class="flex justify-between mt-2 w-full">
    ...
    <div class="w-7/12 flex flex-wrap bg-slate-400 items-start gap-y-6 m-auto pb-7">
        <?php foreach ($posts4 as $post) : ?>
            <div class="h-[225px] flex flex-col items-center w-1/3 px-3">
                <a href=/<?= $post["slug"] ?> class="w-full h-full">
                    <img src=<?= $post["source"] == "Orbital Channel" ? "/images/$post[image]" :  $post["image"] ?> alt="" class="h-5/6 w-full" />
                    <span class="text-[18px] font-bold break-words text-left">
                        <?= mb_convert_case(substr($post["title"], 0, 62), MB_CASE_TITLE, "UTF-8"); ?>...
                    </span>
                </a>
