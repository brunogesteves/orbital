...
$db = new Database();
$post = $db->find("SELECT p.*, i.name as image, u.name as authorName, c.name as category FROM posts p INNER JOIN images i ON i.id = p.image_id INNER JOIN users u ON u.id = p.author_id INNER JOIN categories c ON c.id = p.category_id WHERE p.id=$id");
